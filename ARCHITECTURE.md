...
- **基准宽度不能随便再小**：两处固定布局卡住下限——相册卡片网格 2 列 × `CARD_SIZE.x` + 16 间距、照片网格 4 列 × `THUMB_SIZE`(140) + 3×4。600 下 `CARD_SIZE` 是 (288,276)、卡片网格 592/600、缩略图网格 572/600；600 也是 `THUMB_SIZE` 不用跟着缩的最小值。改基准宽必须同步验这两处（`albums` 的卡片宽是硬约束，早先 648 配 336 的卡片实测会把右列挤出屏幕）。
...
- **缩略图跟着缩放走**：`DeviceMedia.display_scale()`＝物理宽/逻辑宽，`thumb_px()` 把逻辑尺寸折成物理尺寸再解码（P50 上网格 140→286、卡片 288→588），缓存文件名带物理尺寸（`…_286.png`），否则网格里显示的是小图放大后的糊图。云端缩略图由服务端按 512px 生成，不受影响。
